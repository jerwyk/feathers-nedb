...
class Service extends AdapterService {
	...
	async _find(params = {}) {
		...
		params.query &&
			Object.keys(params.query).forEach(
				(key) => params.query[key] === undefined && delete params.query[key]
			);
	...
	_create(raw, params = {}) {
		...
		return nfcall(this.getModel(params), "insert", data).then(
			select(params, this.id)
		);
	}
	...
	_remove(id, params = {}) {
		...
		return this._findOrGet(id, params)
			.then((items) =>
				nfcall(this.getModel(params), "remove", query, options).then(
					() => items
				)
			)
			.then(select(params, this.id));
